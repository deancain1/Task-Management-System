    ...
    public partial class CreateTaskForm : Form
    {
        ...
        public async Task DisplayTaskInDGV(string searchTerm = "")
        {
            ...
            foreach (var task in filteredTasks)
            {
                ...
                DGVTasks.Rows[rowIndex].Cells["StartTime"].Value = task.StartTime.ToString("MMMM-dd-yyyy");
                DGVTasks.Rows[rowIndex].Cells["Deadline"].Value = task.Deadline.ToString("MMMM-dd-yyyy");
        ...
        {
            if (e.ColumnIndex == DGVTasks.Columns["Status"].Index && e.Value != null)
            {
                ...
                switch (status)
                {
                    case "Not Started":
                        ...
                        break;
                    case "Incomplete":
                        e.CellStyle.ForeColor = Color.FromArgb(185, 52, 52);
                        break;
                    case "In Progress":
                        e.CellStyle.ForeColor = Color.MediumSlateBlue;
                        break;
                    case "Completed":
